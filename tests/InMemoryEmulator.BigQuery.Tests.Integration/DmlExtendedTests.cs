...
public class DmlExtendedTests : IAsyncLifetime
{
	...
	public async Task Insert_FromSelect()
	{
		...
		await Exec("INSERT INTO `{ds}.t4a` (id, name) VALUES (1,'A'),(2,'B'),(3,'C')");
	...
	[Fact]
	[Trait(TestTraits.Target, TestTraits.EmulatorDivergence)]
	public async Task Insert_WithExpressions()
	{
		await Exec("CREATE TABLE `{ds}.t5` (id INT64, doubled INT64, label STRING)");
		await Exec("INSERT INTO `{ds}.t5` (id, doubled, label) VALUES (1, 1*2, CONCAT('item_', CAST(1 AS STRING)))");
	...
	public async Task Insert_NullValues()
	{
		...
		await Exec("INSERT INTO `{ds}.t6` (id, val) VALUES (1, NULL)");
		Assert.Null((await Q("SELECT val FROM `{ds}.t6`"))[0]["val"]);
	}

	// ---- UPDATE patterns ----
	// Go emulator requires column list in INSERT VALUES (used in setup).
	// Ref: https://cloud.google.com/bigquery/docs/reference/standard-sql/dml-syntax#insert_statement
	//   "Column names are optional if the target table is not an ingestion-time partitioned table."
	[Fact]
	[Trait(TestTraits.Target, TestTraits.EmulatorDivergence)]
	public async Task Update_AllRows()
	{
		...
		await Exec("INSERT INTO `{ds}.u1` (id, val) VALUES (1,10),(2,20),(3,30)");
	...
	[Fact]
	[Trait(TestTraits.Target, TestTraits.EmulatorDivergence)]
	public async Task Update_WithCondition()
	{
		await Exec("CREATE TABLE `{ds}.u2` (id INT64, status STRING)");
		await Exec("INSERT INTO `{ds}.u2` (id, status) VALUES (1,'active'),(2,'active'),(3,'inactive')");
	...
	public async Task Update_MultipleColumns()
	{
		...
		await Exec("INSERT INTO `{ds}.u3` (id, name, val) VALUES (1,'old',10)");
	...
	public async Task Update_SetToNull()
	{
		...
		await Exec("INSERT INTO `{ds}.u4` (id, val) VALUES (1,10)");
	...
	public async Task Update_WithExpression()
	{
		...
		await Exec("INSERT INTO `{ds}.u5` (id, price, tax) VALUES (1,100,0)");
	...
	public async Task Update_NoMatchingRows()
	{
		...
		await Exec("INSERT INTO `{ds}.u6` (id, val) VALUES (1,10)");
	...
	public async Task Delete_WithCondition()
	{
		...
		await Exec("INSERT INTO `{ds}.d1` (id, val) VALUES (1,10),(2,20),(3,30)");
	...
	public async Task Delete_AllRows()
	{
		...
		await Exec("INSERT INTO `{ds}.d2` (id) VALUES (1),(2),(3)");
	...
	public async Task Delete_NoMatch()
	{
		...
		await Exec("INSERT INTO `{ds}.d3` (id) VALUES (1),(2),(3)");
	...
	[Fact]
	[Trait(TestTraits.Target, TestTraits.EmulatorDivergence)]
	public async Task Delete_WithComplexCondition()
	{
		await Exec("CREATE TABLE `{ds}.d4` (id INT64, grp STRING, val INT64)");
		await Exec("INSERT INTO `{ds}.d4` (id, grp, val) VALUES (1,'A',10),(2,'A',20),(3,'B',30),(4,'B',40)");
	...
	[Fact]
	[Trait(TestTraits.Target, TestTraits.EmulatorDivergence)]
	public async Task Merge_UpdateOnly()
	{
		await Exec("CREATE TABLE `{ds}.m2` (id INT64, name STRING)");
		await Exec("CREATE TABLE `{ds}.m2s` (id INT64, name STRING)");
		await Exec("INSERT INTO `{ds}.m2` (id, name) VALUES (1,'old')");
		await Exec("INSERT INTO `{ds}.m2s` (id, name) VALUES (1,'new')");
		await Exec(@"
			MERGE `{ds}.m2` t
	...
	public async Task Merge_UpsertPattern()
	{
		...
		await Exec("INSERT INTO `{ds}.m3` (id, val) VALUES (1,10)");
		await Exec("INSERT INTO `{ds}.m3s` (id, val) VALUES (1,20),(2,30)");
		await Exec(@"
			MERGE `{ds}.m3` t
	...
	public async Task Merge_DeletePattern()
	{
		...
		await Exec("INSERT INTO `{ds}.m4` (id, active) VALUES (1,true),(2,true),(3,true)");
		await Exec("INSERT INTO `{ds}.m4s` (id) VALUES (2)");
		await Exec(@"
			MERGE `{ds}.m4` t
	...
	public async Task Multi_InsertUpdateDelete()
	{
		...
		await Exec("INSERT INTO `{ds}.ms1` (id, val) VALUES (1,10),(2,20),(3,30)");
	...
	public async Task Multi_InsertThenSelectAggregate()
	{
		...
		await Exec("INSERT INTO `{ds}.ms2` (id, amount) VALUES (1,10.5),(2,20.3),(3,15.2)");
